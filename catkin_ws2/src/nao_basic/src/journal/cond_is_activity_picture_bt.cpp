...
class IsActivityPicture : ROSAction
{
public:
	bool init_;
    ...
    IsActivityPicture(std::string name, std::string robot_ip):
		ROSAction(name),
		init_(false),
	...
	int executeCB(ros::Duration dt)
		{
            std::cout << "**Is Activity Picture-%- Executing Main Task, elapsed_time: "
			          << dt.toSec() << std::endl;
            std::cout << "**Is Activity Picture-%- execute_time: "
            ...
            //set_feedback(RUNNING);
            std::cout <<"Is Picture Game?: " << last_activity_name << std::endl;

            if(last_activity_name.compare("Picture Game") == 0){
                set_feedback(SUCCESS);
                finalize();
                return 1;
                }else{
                behavior_proxy_ptr->stopBehavior("test");
...
{
    std::cout << "Is Activity Picture Started" << std::endl;
    ...
    ros::init(argc, argv, std::string("IsActivityPicture") + "_" + agent); // name used for bt.txt
    IsActivityPicture server(ros::this_node::getName(), robot_ip);
    ros::NodeHandle n;
    ros::Subscriber activity_name =
        n.subscribe<std_msgs::String>("activity" , 1,
                                           &IsActivityPicture::SetActivity, &server);
	ros::spin();
	return 0;
}
