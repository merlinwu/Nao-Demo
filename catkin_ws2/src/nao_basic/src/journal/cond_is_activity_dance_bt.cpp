...
class IsActivityDance : ROSAction
{
public:
	bool init_;
    ...
    IsActivityDance(std::string name, std::string robot_ip):
		ROSAction(name),
		init_(false),
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, std::string("IsActivityDance") + "_" + agent); // name used for bt.txt
    IsActivityDance server(ros::this_node::getName(), robot_ip);
    ros::NodeHandle n;
    ros::Subscriber activity_name =
        n.subscribe<std_msgs::String>("activity" , 1,
                                           &IsActivityDance::SetActivity, &server);
	ros::spin();
	return 0;
}
