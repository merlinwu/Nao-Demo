...
class IsActivitySound : ROSAction
{
public:
	bool init_;
    ...
    IsActivitySound(std::string name, std::string robot_ip):
		ROSAction(name),
		init_(false),
	...
	int executeCB(ros::Duration dt)
		{
            std::cout << "**Is Activity Sound-%- Executing Main Task, elapsed_time: "
			          << dt.toSec() << std::endl;
            std::cout << "**Is Activity Sound-%- execute_time: "
            ...
            set_feedback(RUNNING);
            std::cout <<"Is Sound?: " << last_activity_name << std::endl;

            if(last_activity_name.compare("Sound") == 0){
                set_feedback(SUCCESS);
                finalize();
                return 1;
                }else{
                behavior_proxy_ptr->stopBehavior("sound_tracker");
...
{
    std::cout << "Is Activity Sound Started" << std::endl;
    ...
    ros::init(argc, argv, std::string("IsActivitySound") + "_" + agent); // name used for bt.txt
    IsActivitySound server(ros::this_node::getName(), robot_ip);
    ros::NodeHandle n;
    ros::Subscriber activity_name =
        n.subscribe<std_msgs::String>("activity" , 1,
                                           &IsActivitySound::SetActivity, &server);
	ros::spin();
	return 0;
}
