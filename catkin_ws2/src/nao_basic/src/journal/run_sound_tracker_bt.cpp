...
class RunSoundTracker : ROSAction
{
public:
	bool init_;
    int battery_level;
    ...
    RunSoundTracker(std::string name, std::string robot_ip):
		ROSAction(name),
		init_(false),
	...
	int executeCB(ros::Duration dt)
		{
            std::cout << "**Run Sound Tracker -%- Executing Main Task, elapsed_time: "
			          << dt.toSec() << std::endl;
            std::cout << "**Run Sound Tracker -%- execute_time: "
            ...
            speech_proxy_ptr->say("OK, I'm gonna start the sound tracker");
            speech_rec_proxy_ptr->subscribe("LastWordRecognized");
            behavior_proxy_ptr->stopAllBehaviors();
            behavior_proxy_ptr->runBehavior("sound_tracker");
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, std::string("RunSoundTracker") + "_" + agent); // name used for bt.txt
    RunSoundTracker server(ros::this_node::getName(), robot_ip);
	ros::spin();
	return 0;
}
