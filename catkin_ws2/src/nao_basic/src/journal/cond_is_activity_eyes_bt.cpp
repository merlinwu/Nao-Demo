...
class IsActivityEyes : ROSAction
{
public:
	bool init_;
    ...
    IsActivityEyes(std::string name, std::string robot_ip):
		ROSAction(name),
		init_(false),
	...
	int executeCB(ros::Duration dt)
		{
            std::cout << "**Is Activity Eyes-%- Executing Main Task, elapsed_time: "
			          << dt.toSec() << std::endl;
            std::cout << "**Is Activity Eyes-%- execute_time: "
           ...
           // set_feedback(RUNNING);
            std::cout <<"Is Eyes?: " << last_activity_name << std::endl;

            if(last_activity_name.compare("Eyes") == 0){
                set_feedback(SUCCESS);
                finalize();
                return 1;
                }else{
                behavior_proxy_ptr->stopBehavior("mov_tracker");
...
{
    std::cout << "Is Activity Eyes Started" << std::endl;
    ...
    ros::init(argc, argv, std::string("IsActivityEyes") + "_" + agent); // name used for bt.txt
    IsActivityEyes server(ros::this_node::getName(), robot_ip);
    ros::NodeHandle n;
    ros::Subscriber activity_name =
        n.subscribe<std_msgs::String>("activity" , 1,
                                           &IsActivityEyes::SetActivity, &server);
	ros::spin();
	return 0;
}
