...
class IsActivitySleep : ROSAction
{
public:
	bool init_;
    ...
    IsActivitySleep(std::string name, std::string robot_ip):
		ROSAction(name),
		init_(false),
	...
	int executeCB(ros::Duration dt)
		{
            std::cout << "**Is Activity Sleep-%- Executing Main Task, elapsed_time: "
			          << dt.toSec() << std::endl;
            std::cout << "**Is Activity Sleep-%- execute_time: "
            ...
            if(last_activity_name.compare("Sleep") == 0){
...
{
    std::cout << "Is Activity Stand Started" << std::endl;
    ...
    ros::init(argc, argv, std::string("IsActivitySleep") + "_" + agent); // name used for bt.txt
    IsActivitySleep server(ros::this_node::getName(), robot_ip);
    ros::NodeHandle n;
    ros::Subscriber activity_name =
        n.subscribe<std_msgs::String>("activity" , 1,
                                           &IsActivitySleep::SetActivity, &server);
	ros::spin();
	return 0;
}
