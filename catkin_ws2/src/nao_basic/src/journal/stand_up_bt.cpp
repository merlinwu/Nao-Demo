...
public:
    ...
    StandUp(std::string name, std::string robot_ip):
		ROSAction(name),
		init_(false),
	...
	int executeCB(ros::Duration dt)
		{
            std::cout << "**Stand Up -%- Executing Main Task, elapsed_time: "
			          << dt.toSec() << std::endl;
            std::cout << "**Stand Up -%- execute_time: "
            ...
            posture_proxy_ptr->goToPosture("Stand", 0.5);
...
{
    std::cout << "Stand Up Started" << std::endl;
    ...
    ros::init(argc, argv, std::string("StandUp") + "_" + agent); // name used for bt.txt
    StandUp server(ros::this_node::getName(), robot_ip);
	ros::spin();
	return 0;
}
