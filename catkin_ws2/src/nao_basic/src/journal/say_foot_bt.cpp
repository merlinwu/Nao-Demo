...
public:
    ...
    SayFoot(std::string name, std::string robot_ip):
		ROSAction(name),
		init_(false),
	...
	int executeCB(ros::Duration dt)
		{
            std::cout << "**Say Foot -%- Executing Main Task, elapsed_time: "
			          << dt.toSec() << std::endl;
            std::cout << "**Say Foot -%- execute_time: "
            ...
            speech_proxy_ptr->say("Ouch, my toe");
...
{
    std::cout << "Say Foot Started" << std::endl;
    ...
    ros::init(argc, argv, std::string("SayFoot") + "_" + agent); // name used for bt.txt
    SayFoot server(ros::this_node::getName(), robot_ip);
	ros::spin();
	return 0;
}
