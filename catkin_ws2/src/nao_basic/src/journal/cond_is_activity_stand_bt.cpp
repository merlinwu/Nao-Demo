...
class IsActivityStand : ROSAction
{
public:
	bool init_;
    ...
    IsActivityStand(std::string name, std::string robot_ip):
		ROSAction(name),
		init_(false),
	...
	int executeCB(ros::Duration dt)
		{
            ...
            if(last_activity_name.compare("Stand Up") == 0){
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, std::string("IsActivityStand") + "_" + agent); // name used for bt.txt
    IsActivityStand server(ros::this_node::getName(), robot_ip);
    ros::NodeHandle n;
    ros::Subscriber activity_name =
        n.subscribe<std_msgs::String>("activity" , 1,
                                           &IsActivityStand::SetActivity, &server);
	ros::spin();
	return 0;
}
