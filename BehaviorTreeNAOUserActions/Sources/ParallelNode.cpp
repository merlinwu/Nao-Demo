...
ParallelNode::ParallelNode(std::string Name) : ControlNode::ControlNode(Name)
{
    ...
    // Thread start
    Thread = new std::thread(&ParallelNode::Exec, this);
    Thread->detach();
}
...
void ParallelNode::SetThreashold(unsigned int N)
{
    ...
    if (N > M)
    {
        throw BehaviorTreeException("Wrong N threshold for '" + Name + "'. M=" + std::to_string(M) + " while N=" + std::to_string(N) + ". N should be <= M.");
    }
